...
model = Model("model-carrere.json")
task_learning  = Task("task-reversal.json")

print("-"*30)
print("Seed:     %d" % seed)
print("Model:    %s" % model.filename)
print("Task:     %s" % task_learning.filename)
print("-"*30)


trial_learning = task_learning[0]
model.process(task_learning, trial_learning, stop=False,debug=True)


cog = model["CTX"]["cog"].history[:3000]
mot = model["CTX"]["mot"].history[:3000]
NE_amy = model["NE"]["amy"].history[:3000]
NE_hip = model["NE"]["hip"].history[:3000]
NE = model["NE"]["out"].history[:3000]
...
plt.plot(timesteps, NE_amy[:,0], c='g', label="NE long")
plt.plot(timesteps, NE_hip[:,0], c='k', label="NE short")
plt.plot(timesteps, NE[:,0], c='y', label="NE release")
...
plt.savefig("single-trial-GPi-ON.pdf")
plt.show()
